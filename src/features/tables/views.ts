...
export function rowsView(opts: {
  ...
  const insertInputs = insertCols
    .map(
      (c) => `<label style="display:block;margin-bottom:0.5rem">
    <span style="font-size:0.8rem;font-weight:600">${c.name}</span>
    <input data-bind:${c.name} placeholder="${c.type || "text"}">
  </label>`,
    )
    .join("");

  const colSignals = insertCols.map((c) => `${c.name}:''`).join(",");
  const signalsAttr = `{_showInsert:false${colSignals ? "," + colSignals : ""}}`;
  return String(
    html`<div data-signals="${raw(signalsAttr)}">
        <div
          style="display:flex;align-items:center;gap:1rem;margin-bottom:1rem"
        >
          <h2>${table}</h2>
          <button class="primary" data-on:click="$_showInsert=!$_showInsert">
            + Add row
          </button>
        </div>
        <div
