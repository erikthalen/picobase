...
export function getPendingForTable(
  db: DatabaseSync,
  tableName: string,
): DesiredColumn[] | null {
  const row = db
...
export function generateDiffSQL(
  ...
  const desiredOriginals = new Set(
    desired.map((d) => d.originalName).filter((n) => n !== ""),
  );
  ...
  const renamed = existing.filter(
    (d) =>
      ...
      !modified.some((m) => m.originalName === d.originalName),
  );

  if (!added.length && !dropped.length && !renamed.length && !modified.length)
    return "";
  ...
  for (const col of renamed) {
    stmts.push(
      `ALTER TABLE ${quoteIdent(tableName)} RENAME COLUMN ${quoteIdent(col.originalName)} TO ${quoteIdent(col.name)};`,
    );
  }
  for (const col of dropped) {
    stmts.push(
      `ALTER TABLE ${quoteIdent(tableName)} DROP COLUMN ${quoteIdent(col.name)};`,
    );
  }
  for (const col of added) {
    stmts.push(
      `ALTER TABLE ${quoteIdent(tableName)} ADD COLUMN ${buildColumnDef(col)};`,
    );
  }
  return stmts.join("\n");
}
