const KEYWORDS = new Set([
	...
	"ROWS", "SAVEPOINT", "SELECT", "SET", "STATEMENT", "TABLE", "TEMP",
	...
	// Types
	"BLOB", "BOOLEAN", "CHAR", "DATE", "DATETIME", "DOUBLE", "FLOAT", "INT",
	"INTEGER", "NUMERIC", "REAL", "TEXT", "TIMESTAMP", "VARCHAR",
]);
...
const TOKEN = new RegExp(
	[
		...
		String.raw`(` + "`" + String.raw`(?:[^` + "`" + String.raw`\\]|\\.)*` + "`" + `)`, // 3 backtick identifier
		String.raw`(\[(?:[^\]\\]|\\.)*\])`,        // 4 bracket identifier
		String.raw`(--[^\n]*)`,                    // 5 line comment
		String.raw`(\/\*[\s\S]*?\*\/)`,            // 6 block comment
		String.raw`((?:\d+\.?\d*|\.\d+)(?:[eE][+-]?\d+)?)`, // 7 number
		String.raw`([A-Za-z_][A-Za-z0-9_]*)`,     // 8 word
		String.raw`([ \t\n\r]+)`,                  // 9 whitespace
		String.raw`(.)`,                           // 10 anything else
	].join("|"),
	"g",
);
