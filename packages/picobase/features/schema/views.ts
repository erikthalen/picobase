...
export function colRowHtml(
	...
	const typeOptions = COL_TYPES.map(
		(t) => `<option value="${t}"${t === type ? " selected" : ""}>${t}</option>`,
	).join("");
	const safeName = name.replace(/\\/g, "\\\\").replace(/'/g, "\\'");
	const safeDflt = dflt.replace(/\\/g, "\\\\").replace(/'/g, "\\'");
	const removeBtn = removable
		? `<button type="button" style="padding:0.25rem 0.5rem;background:transparent;border:1px solid var(--pb-border-input);border-radius:4px;color:var(--pb-text-muted);font-size:0.75rem;cursor:pointer;line-height:1;flex-shrink:0;" data-on:click="@delete('${base}/schema/new-col/${i}')">×</button>`
		: `<span></span>`;
	return `<div id="new-col-${i}" data-signals="{col${i}name: '${safeName}', col${i}type: '${type}', col${i}dflt: '${safeDflt}', col${i}pk: ${pk}}" style="display:grid;grid-template-columns:1fr 1fr 1fr auto auto;gap:0.375rem;align-items:center;">
  <input data-bind:col${i}name placeholder="column_name"${name ? ` value="${name}"` : ""}>
  <select data-bind:col${i}type>${typeOptions}</select>
  <input data-bind:col${i}dflt placeholder="NULL"${dflt ? ` value="${dflt}"` : ""}>
  <input type="checkbox" data-bind:col${i}pk${pk ? " checked" : ""} style="width:1rem;height:1rem;cursor:pointer;margin:0 auto;">
  ${removeBtn}
</div>`;
}
...
const header = html`
  <nav id="table-tabs" class="tab-bar">
    <a href="/schema/diagram">Diagram</a>
    <a href="/schema">Schema</a>
  </nav>
`;

export function schemaListView(schema: TableSchema[]): string {
	const tables = schema
		.map((t) => {
...
export function erDiagramView(schema: TableSchema[], basePath: string): string {
	...
	const PAD = 60;
	const DRAG_EXTRA = 10; // extra canvas room for dragging
	...
	const svgBoxes = schema.map((t) => {
		const pos = positions[t.name] ?? { x: 0, y: 0, h: 0 };
		const colRows = t.columns.map((c, ci) => {
			...
			const alt =
				ci === t.columns.length - 1
					? html`<path
              d="M0 ${BOX_HEADER_H + ci * ROW_H} H${BOX_W} V${
								BOX_HEADER_H + ci * ROW_H + ROW_H - 8
							} A8 8 0 0 1 ${BOX_W - 8} ${
								BOX_HEADER_H + ci * ROW_H + ROW_H
							} H8 A8 8 0 0 1 0 ${BOX_HEADER_H + ci * ROW_H + ROW_H - 8} Z"
              fill="${
								ci % 2 === 1 ? "var(--pb-diagram-row-alt)" : "var(--pb-bg)"
							}"
            />`
					: html`<rect
              x="0"
    ...
    (function () {
      ...
      svg.addEventListener("mousedown", function (e) {
        ...
        if (!g) return;
        // Always read position from DOM — pos may be stale after SSE re-renders
        var mm = /translate\\(([-\\d.]+),([-\\d.]+)\\)/.exec(g.getAttribute("transform"));
        if (!mm) return;
        pos[g.dataset.table] = { x: parseFloat(mm[1]), y: parseFloat(mm[2]), h: parseFloat(g.dataset.h) };
        drag = g;
	...
	return String(
		html`${header}
      <div data-signals="{tableName: '', _colIdx: 1}" style="position:relative;height: 100%;">
        <button
        ...
        <dialog
          ...
          closedby="any"
          style="left:50%;top:50%;translate:-50% -50%;background:var(--pb-surface);border:1px solid var(--pb-border);padding:0;width:min(680px,95vw);max-height:85vh;overflow:hidden;color:var(--pb-text-heading);"
        >
          <div style="display:flex;align-items:center;justify-content:space-between;padding:1.125rem 1.5rem;border-bottom:1px solid var(--pb-border);flex-shrink:0;">
            <span style="font-size:0.9375rem;font-weight:600;">New Table</span>
            <button type="button" style="background:none;border:none;color:var(--pb-text-muted);cursor:pointer;padding:0.25rem;font-size:1.25rem;line-height:1;" data-on:click="$_createTableDialog.close(); tableName = ''">×</button>
          </div>
          <div style="overflow-y:auto;flex:1;padding:1.25rem 1.5rem;display:flex;flex-direction:column;gap:1.5rem;">
            <div style="display:grid;grid-template-columns:6rem 1fr;align-items:center;gap:0.75rem;">
              <label style="font-size:0.8125rem;font-weight:500;color:var(--pb-text-muted);">Name</label>
              <input type="text" data-bind:table-name placeholder="e.g. orders" autofocus>
            </div>
            <div>
              <p style="font-size:0.8125rem;font-weight:600;margin-bottom:0.75rem;">Columns</p>
              <div style="display:grid;grid-template-columns:1fr 1fr 1fr auto auto;gap:0.375rem;padding:0 0.25rem;margin-bottom:0.5rem;">
                <span style="font-size:0.6875rem;font-weight:500;color:var(--pb-text-muted);letter-spacing:0.06em;text-transform:uppercase;">Name</span>
                <span style="font-size:0.6875rem;font-weight:500;color:var(--pb-text-muted);letter-spacing:0.06em;text-transform:uppercase;">Type</span>
                <span style="font-size:0.6875rem;font-weight:500;color:var(--pb-text-muted);letter-spacing:0.06em;text-transform:uppercase;">Default</span>
                <span style="font-size:0.6875rem;font-weight:500;color:var(--pb-text-muted);letter-spacing:0.06em;text-transform:uppercase;">PK</span>
                <span></span>
              </div>
              <div id="dialog-columns" style="display:flex;flex-direction:column;gap:0.375rem;">
                ${raw(colRowHtml(0, base, { name: "id", type: "INTEGER", pk: true, removable: false }))}
              </div>
              <button type="button" style="margin-top:0.75rem;" data-on:click="@get('${base}/schema/new-col?i=' + $_colIdx); $_colIdx = $_colIdx + 1">+ Add column</button>
            </div>
          </div>
          <div style="display:flex;justify-content:flex-end;gap:0.5rem;padding:1rem 1.5rem;border-top:1px solid var(--pb-border);flex-shrink:0;">
            <button type="button" data-on:click="$_createTableDialog.close(); tableName = ''">Cancel</button>
            <button type="button" class="primary" data-on:click="@post('${base}/schema/tables'); $_createTableDialog.close(); tableName = ''; $_colIdx = 1">Create Table</button>
          </div>
        </dialog>

        <div id="schema-content" style="overflow:auto;height: 100%;">
          <svg
            width="${svgW}"
            height="${svgH}"
            xmlns="http://www.w3.org/2000/svg"
            style="background:var(--pb-diagram-bg);display:block;min-width: 100%;min-height: 100%;"
          >
