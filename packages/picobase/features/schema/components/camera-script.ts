...
export function cameraScript(BOX_W: number, BOX_HEADER_H: number, ROW_H: number) {
  return html`<script>
    (function () {
      var container = document.getElementById("diagram-viewport");
      var wrap = document.getElementById("canvas-wrap");
      var BW = ${BOX_W}, BH = ${BOX_HEADER_H}, RH = ${ROW_H};
      ...
      function zoomCamera(cx, cy, dz) {
        ...
        var p1x = cx / z1 - camera.x, p1y = cy / z1 - camera.y;
        var p2x = cx / z2 - camera.x, p2y = cy / z2 - camera.y;
        camera = { x: camera.x + (p2x - p1x), y: camera.y + (p2y - p1y), z: z2 };
        applyCamera();
      }
      ...
      container.addEventListener("wheel", function (e) {
        ...
        if (e.ctrlKey || e.metaKey) {
          zoomCamera(e.clientX - rect.left, e.clientY - rect.top, e.deltaY / 100);
        } else {
          camera = {
      ...
      function isHB(x1, x2, y) {
        var xl = x1<x2?x1:x2, xr = x1>x2?x1:x2;
        for (var k in pos) { var p=pos[k]; if (xl<p.x+BW-2&&xr>p.x+2&&y>p.y+2&&y<p.y+p.h-2) return true; }
        return false;
      }
      function isVB(x, ya, yb) {
        var yt = ya<yb?ya:yb, ybt = ya>yb?ya:yb;
        for (var k in pos) { var p=pos[k]; if (x>p.x+2&&x<p.x+BW-2&&yt<p.y+p.h-2&&ybt>p.y+2) return true; }
        return false;
      }
      function bp(f, t, fc, tc, lo) {
        var ST=10, R=16, MARGIN=24; lo=lo||0;
        var y1=f.y+BH+fc*RH+RH/2, y2=t.y+BH+tc*RH+RH/2;
        var sv=y2>=y1?1:-1, sh=t.x>=f.x?1:-1;
        var px1=sh>0?f.x+BW:f.x, px2=sh>0?t.x:t.x+BW;
        ...
        if (!(adx>=2)||willFold) {
          ...
          cx+=lo;
          var vd=y2>y1?y2-y1:y1-y2, r=vd/2>R?R:vd/2;
          if (!(r>0.5)) return "M"+(f.x+BW)+","+y1+" H"+cx+" H"+(t.x+BW);
          var ud="M"+(f.x+BW)+","+y1+" H"+(cx-r)+" Q"+cx+","+y1+" "+cx+","+(y1+sv*r);
          if (vd-2*r>0.5) ud+=" V"+(y2-sv*r);
          ud+=" Q"+cx+","+y2+" "+(cx-r)+","+y2+" H"+(t.x+BW);
          return ud;
        }
        var ax=px1+sh*ST, bx=px2-sh*ST;
        var yMin=y1<y2?y1:y2, yMax=y1>y2?y1:y2;
        if (!(Math.abs(y2-y1)>=2)&&!isHB(ax,bx,y1)) return "M"+px1+","+y1+" H"+px2;
        var vx=(ax+bx)/2;
        for (var iter=0;iter<8;iter++) {
          for (var k in pos) {
            var bb=pos[k];
            if (vx>bb.x+2&&bb.x+BW-2>vx&&bb.y+bb.h>yMin&&yMax>bb.y) {
              vx=vx-bb.x<=bb.x+BW-vx?bb.x-R:bb.x+BW+R;
            }
          }
        }
        if (sh>0){if(!(vx>=ax+R))vx=ax+R;if(vx>bx-R)vx=bx-R;}
        else    {if(vx>ax-R)vx=ax-R;if(!(vx>=bx+R))vx=bx+R;}
        vx+=lo;
        if (!isHB(ax,vx,y1)&&!isVB(vx,y1,y2)&&!isHB(vx,bx,y2)) {
          var r2=Math.abs(y2-y1)/2>R?R:Math.abs(y2-y1)/2;
          var c1x=vx-sh*r2, c1y=y1+sv*r2, c2y=y2-sv*r2, c2x=vx+sh*r2;
          var d="M"+px1+","+y1+" H"+ax;
        ...
        var xL=(ax<bx?ax:bx)-R, xR=(ax>bx?ax:bx)+R;
        var topY=Infinity, botY=-Infinity;
        for (var j in pos) { var p2=pos[j]; if (p2.x+BW>xL&&p2.x<xR) { if(p2.y<topY)topY=p2.y; if(p2.y+p2.h>botY)botY=p2.y+p2.h; } }
        var safeYA=(topY===Infinity?y1:topY)-MARGIN;
        var safeYB=(botY===-Infinity?y1:botY)+MARGIN;
        var safeY=(Math.abs(y1-safeYA)+Math.abs(y2-safeYA)<=Math.abs(y1-safeYB)+Math.abs(y2-safeYB)?safeYA:safeYB)+lo;
        return buildCP([{x:px1,y:y1},{x:ax,y:y1},{x:ax,y:safeY},{x:bx,y:safeY},{x:bx,y:y2},{x:px2,y:y2}],R);
      }
      ...
      function updatePaths() {
        ...
        wrap.querySelectorAll("path[data-from]").forEach(function(p) {
          var f=pos[p.dataset.from], t=pos[p.dataset.to];
          if (!f||!t) return;
          var fc=+p.dataset.fromCol, tc=+p.dataset.toCol;
          var vx=computeVx(f,t,fc,tc);
          var y1=f.y+BH+fc*RH+RH/2, y2=t.y+BH+tc*RH+RH/2;
          entries.push({el:p, f:f, t:t, fc:fc, tc:tc, vx:vx, y1:y1, y2:y2, lo:0});
        });
        ...
        for (var i=0;i<entries.length;i++) {
          ...
          for (var j=i+1;j<entries.length;j++) {
            ...
            var aMin=a.y1<a.y2?a.y1:a.y2, aMax=a.y1>a.y2?a.y1:a.y2;
            var bMin=b.y1<b.y2?b.y1:b.y2, bMax=b.y1>b.y2?b.y1:b.y2;
            if (Math.abs(a.vx-b.vx)<2 && Math.min(aMax,bMax)>Math.max(aMin,bMin)) {
              group.push(j); used[j]=true;
            }
          }
          var n=group.length;
          for (var r=0;r<n;r++) entries[group[r]].lo = n>1?(r-(n-1)/2)*10:0;
        }
        for (var i=0;i<entries.length;i++) {
          var e=entries[i];
      ...
      var isDragging = false, dragEl = null, dragStart = null, dragElStart = null;
      var isPanning = false, panStart = null, cameraAtStart = null;
      ...
      wrap.addEventListener("mousedown", function (e) {
        ...
        if (header) {
          ...
          dragElStart = { x: pos[box.dataset.table].x, y: pos[box.dataset.table].y };
      ...
      window.addEventListener("mousemove", function (e) {
        if (isDragging) {
          ...
          var nx = dragElStart.x + dx, ny = dragElStart.y + dy;
          pos[dragEl.dataset.table] = { x: nx, y: ny, h: pos[dragEl.dataset.table].h };
      ...
      // ── Dash animation ────────────────────────────────────────
      var dashOffset = 0, dashLastTs = null;
      function animateDash(ts) {
        if (dashLastTs !== null) dashOffset -= (14 / 0.75) * (ts - dashLastTs) / 1000 / camera.z;
        dashLastTs = ts;
        wrap.querySelectorAll("path[data-from]").forEach(function (p) {
